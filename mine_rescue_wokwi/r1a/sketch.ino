/*
 * =====================================================================
 * SIH 2026 MINE RESCUE ROVER - R1A ENTRY RELAY NODE
 * =====================================================================
 * Microcontroller : ESP32 DevKit V1 (Wokwi Simulation)
 * Role            : Redundant Entry Relay Branch A
 *
 * Subscribes to:
 *   - Topic: mine/rover/to/r1a
 * Forwards to:
 *   - Topic: mine/r1a/to/r2
 * Heartbeat:
 *   - Topic: node/status/R1A
 *
 * Hardware connections:
 *   - GPIO 2  : Packet Forwarding Activity LED (Green)
 *   - GPIO 4  : Heartbeat / Power LED (Blue / Yellow)
 ...
 *   s = display relay statistics (forwarded, dropped, duplicates)
 *   h = help menu
 * =====================================================================
 */
...
const char* NODE_ID       = "R1A";

// MQTT Topics
const char* TOPIC_SUB     = "mine/rover/to/r1a";
const char* TOPIC_PUB     = "mine/r1a/to/r2";
const char* TOPIC_STATUS  = "node/status/R1A";
...
// Duplicate Detection Ring Buffer (Cache last 40 sequence numbers)
...
void publishHeartbeat() {
  if (isFaultOffline) return; // Do not send heartbeat if node is offline
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // If node is simulated as offline/dead, drop all packets
  if (isFaultOffline) {
    packetsDropped++;
    digitalWrite(PIN_LED_FAULT, HIGH);
    delay(30);
    digitalWrite(PIN_LED_FAULT, LOW);
    Serial.println("[R1A] [OFFLINE] Packet dropped because node is simulated FAULT/OFFLINE.");
    return;
  }
  ...
  if (err) {
    packetsDropped++;
    Serial.printf("[R1A] [ERROR] JSON deserialization failed: %s\n", err.c_str());
    return;
  }
  ...
  if (isDuplicate(seq)) {
    duplicatesCount++;
    packetsDropped++;
    Serial.printf("[R1A] [DUPLICATE DROPPED] Seq #%lu already forwarded. Ignored.\n", seq);
    digitalWrite(PIN_LED_FAULT, HIGH);
    delay(20);
    digitalWrite(PIN_LED_FAULT, LOW);
    return;
  ...
  // Mark sequence number as seen
  addToCache(seq);

  // Preserve all payload fields and append relay to path
  ...
  // Serialize and forward packet to downstream relay
  char outBuffer[512];
  ...
  if (success) {
    packetsForwarded++;
    Serial.println("--------------------------------------------------");
    Serial.printf("[R1A FORWARDED] Seq #%lu | Path: %s\n", seq, currentPath.c_str());
    Serial.printf("  Sub: %s  ==>  Fwd: %s\n", topic, TOPIC_PUB);
  } else {
    packetsDropped++;
    Serial.println("[R1A] [ERROR] MQTT publish to R2 failed!");
  }
}
...
void handleSerialInput() {
  if (Serial.available() > 0) {
    ...
    if (cmd == 'f' || cmd == 'F') {
      ...
      if (isFaultOffline) {
        digitalWrite(PIN_LED_FAULT, HIGH);
        Serial.println("\n[SIMULATION] >>> R1A IS NOW OFFLINE (SIMULATED FAILURE)! <<<");
        Serial.println("Rover packets on Branch A will be DROPPED. Branch B (R1B) will take over.");
      } else {
        digitalWrite(PIN_LED_FAULT, LOW);
        Serial.println("\n[SIMULATION] >>> R1A RESTORED TO ONLINE STATE! <<<");
      }
    } else if (cmd == 's' || cmd == 'S') {
      printStatus();
    } else if (cmd == 'h' || cmd == 'H') {
...
// Hardware button debounce to toggle fault
...
void checkFaultButton() {
  if (digitalRead(PIN_BTN_FAULT) == LOW) {
    if (millis() - lastBtnPressMs > 400) {
      ...
      Serial.printf("\n[BUTTON] Fault button toggled R1A state: %s\n",
...
void setup() {
  ...
  delay(500);
  Serial.println("\n=============================================");
  Serial.println("  SIH 2026 MINE RESCUE ROVER - R1A RELAY     ");
  Serial.println("=============================================");
