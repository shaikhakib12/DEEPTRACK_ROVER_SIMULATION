/*
 * =====================================================================
 * SIH 2026 MINE RESCUE ROVER - R4A EXIT RELAY NODE
 * =====================================================================
 * Microcontroller : ESP32 DevKit V1 (Wokwi Simulation)
 * Role            : Exit Relay Branch A (Interfaces with Gateway)
 *
 * Subscribes to:
 *   - Topic: mine/r2/to/r4a
 * Forwards to:
 *   - Topic: mine/r4a/to/gateway
 * Heartbeat:
 *   - Topic: node/status/R4A
 *
 ...
 *   - Checks path: if "R4A" is already present, packet is DROPPED.
 * =====================================================================
 */
...
const char* NODE_ID       = "R4A";

// MQTT Topics
const char* TOPIC_SUB     = "mine/r2/to/r4a";
const char* TOPIC_PUB     = "mine/r4a/to/gateway";
const char* TOPIC_STATUS  = "node/status/R4A";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (isFaultOffline) {
    ...
    Serial.println("[R4A] [OFFLINE] Packet dropped (simulated failure mode).");
    return;
  }
  ...
  if (err) {
    packetsDropped++;
    Serial.printf("[R4A] [ERROR] JSON deserialization failed: %s\n", err.c_str());
    return;
  }
  ...
  // Loop Prevention
  if (currentPath.indexOf("R4A") != -1) {
    packetsDropped++;
    Serial.printf("[R4A] [LOOP DETECTED] Packet already visited R4A (Path: %s). Dropped.\n", currentPath.c_str());
    digitalWrite(PIN_LED_FAULT, HIGH);
    delay(20);
    digitalWrite(PIN_LED_FAULT, LOW);
    return;
  ...
  if (isDuplicate(packetId)) {
    duplicatesCount++;
    packetsDropped++;
    Serial.printf("[R4A] [DUPLICATE] Packet ID '%s' already handled. Dropped.\n", packetId.c_str());
    digitalWrite(PIN_LED_FAULT, HIGH);
    delay(20);
    digitalWrite(PIN_LED_FAULT, LOW);
    return;
  ...
  if (success) {
    packetsForwarded++;
    Serial.println("--------------------------------------------------");
    Serial.printf("[R4A FORWARDED TO GATEWAY] Seq #%lu | Path: %s\n", seq, currentPath.c_str());
    Serial.printf("  Sub: %s  ==>  Pub: %s\n", topic, TOPIC_PUB);
  } else {
    packetsDropped++;
    Serial.println("[R4A] [ERROR] MQTT publish to Gateway failed!");
  }
}
...
void handleSerialInput() {
  if (Serial.available() > 0) {
    ...
    if (cmd == 'f' || cmd == 'F') {
      ...
      if (isFaultOffline) {
        digitalWrite(PIN_LED_FAULT, HIGH);
        Serial.println("\n[SIMULATION] >>> R4A IS NOW OFFLINE (SIMULATED FAILURE)! <<<");
        Serial.println("Exit Relay R4A will drop packets. Exit Relay R4B will sustain delivery to Gateway.");
      } else {
        digitalWrite(PIN_LED_FAULT, LOW);
        Serial.println("\n[SIMULATION] >>> R4A RESTORED TO ONLINE STATE! <<<");
      }
    } else if (cmd == 's' || cmd == 'S') {
      printStatus();
    } else if (cmd == 'h' || cmd == 'H') {
...
void checkFaultButton() {
  if (digitalRead(PIN_BTN_FAULT) == LOW) {
    if (millis() - lastBtnPressMs > 400) {
      ...
      Serial.printf("\n[BUTTON] Fault button toggled R4A state: %s\n",
...
void setup() {
  ...
  delay(500);
  Serial.println("\n=============================================");
  Serial.println("  SIH 2026 MINE RESCUE ROVER - R4A EXIT RELAY");
  Serial.println("=============================================");
